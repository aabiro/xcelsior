...
Usage:
  ...
  python scripts/volumes_worker_mount_smoke.py --host 100.64.0.6
  AARYNFANS_PROD_HOST=100.64.x.x python scripts/volumes_worker_mount_smoke.py

Mounts Mac NFS export from the worker the same way worker_agent does at job start.
...
def main() -> int:
    ...
    parser.add_argument(
        "--host",
        default=os.environ.get("AARYNFANS_PROD_HOST")
        or os.environ.get("WORKER_MOUNT_HOST")
        or "100.64.0.6",
        help="Worker mesh IP (default: AARYNFANS_PROD_HOST or ASUS 100.64.0.6)",
    )
    parser.add_argument(
        "--ssh-user",
        default=os.environ.get("XCELSIOR_SSH_USER", "xcelsior"),
    )
    parser.add_argument(
        "--ssh-key",
