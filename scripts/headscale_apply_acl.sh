...
# VPS uses policy.mode=file — copy acl.json and restart (policy set only works in database mode).
ssh_cmd "sudo cp /tmp/xcelsior-acl.hujson /etc/headscale/acl.json && sudo systemctl restart headscale && sudo systemctl restart tailscaled"
