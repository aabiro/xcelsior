...
def find_helper_functions(source, lines, tree, route_ranges):
    ...
    for node in ast.walk(tree):
        ...
        if node.name in SKIP_FUNCTIONS:
            continue
        if node.name.startswith("_bg_") or node.name in (
...
def main():
    ...
    modules = collect_module_items(
        routes, models, helpers, model_assignments, helper_assignments, lines
    )
