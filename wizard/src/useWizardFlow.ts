    ...
    const advanceToNext = useCallback(
        (currentAnswers: Record<string, string | string[]>) => {
            ...
            if (nextStep.type === "auto-check" && nextStep.checkId) {
                const choreographyState: WizardState = nextStep.checkId === "launch" ? "thinking" : "thinking";
                setWizardState(choreographyState);
                ...
                runCheck(nextStep.checkId, currentAnswers).then((results) => {
                    ...
                    if (allPassed) {
                        setWizardState(nextStep.checkId === "launch" ? "success" : "success");
                        setWizardMessage(
                            nextStep.checkId === "launch"
                                ? "Instance launched!"
                                : "All checks passed!",
                        );
