...
**87 of 373 routes (23%)** and **17 of 51 CLI commands** (33%) have no test signal.
...
### `routes/agent.py` (3 untested)
- [x] `POST /agent/benchmark` — `api_agent_benchmark`
- [ ] `GET /agent/commands/{host_id}` — `api_agent_commands_drain`
...
- [ ] `GET /api/artifacts/{job_id}` — `api_list_artifacts`
- [ ] `GET /api/artifacts/{job_id}/expiry` — `api_artifact_expiry`

### `routes/auth.py` (6 untested)
...
- [ ] `DELETE /api/keys/{key_preview}` — `api_revoke_key`
...
- [ ] `DELETE /api/oauth/clients/{client_id}` — `api_delete_oauth_client`
- [ ] `PATCH /api/oauth/clients/{client_id}` — `api_update_oauth_client`
...
- [ ] `GET /api/slurm/status/{slurm_job_id}` — `api_slurm_cancel`
- [x] `POST /api/slurm/submit` — `api_slurm_status`
- [ ] `DELETE /api/slurm/{slurm_job_id}` — `api_slurm_profiles`
...
- [ ] `POST /build/{model}/dockerfile` — `_sse_generator`
...
- [ ] `GET /legacy/{path:path}` — `api_get_alert_config`
...
- [ ] `GET /host/{host_id}/maintenance` — `api_host_maintenance`
- [ ] `POST /host/{host_id}/undrain` — `api_undrain_host`
...
### `routes/instances.py` (2 untested)
...
- [ ] `POST /api/instances/{job_id}/stream-ticket` — `api_instance_stream_ticket`
...
- [ ] `POST /instances/{job_id}/auto-launch/report` — `api_instances_auto_launch_report`
...
### `routes/marketplace.py` (4 untested)
...
- [ ] `DELETE /api/v2/marketplace/reservations/{reservation_id}` — `api_marketplace_cancel_reservation`
...
### `routes/notifications.py` (2 untested)
...
- [ ] `GET /api/privacy/consent/{entity_id}` — `api_get_consents`
- [ ] `DELETE /api/privacy/consent/{entity_id}/{consent_type}` — `api_revoke_consent`
...
### `routes/providers.py` (4 untested)
...
### `routes/reputation.py` (4 untested)
...
- [ ] `GET /api/reputation/{entity_id}` — `api_get_reputation`
- [ ] `GET /api/reputation/{entity_id}/breakdown` — `api_reputation_breakdown`
- [ ] `GET /api/reputation/{entity_id}/history` — `api_reputation_history`
- [x] `GET /api/trust-tiers` — `api_trust_tiers`

### `routes/sla.py` (2 untested)
...
- [ ] `GET /api/sla/{host_id}` — `api_sla_status`
...
### `routes/stripe_connect_v2.py` (2 untested)
...
- [ ] `DELETE /api/teams/{team_id}/members/{email}` — `api_remove_team_member`
- [ ] `PATCH /api/teams/{team_id}/members/{email}` — `api_update_team_member_role`
