...
Registry is mandatory in production. Phase E deploys the registry infra + per-user creds management. Local-only fallback is removed; snapshot_container returns status=failed when XCELSIOR_REGISTRY_URL is unset.
Rate limit is in-process (no Redis); acceptable for snapshots since disk is host-local anyway.
Phase D (frontend templates UI) and Phase E (registry infra) follow Phase 3 hardening — see the bottom of this doc.
...
Phase D — Frontend Templates UI (Comprehensive Organization & Management)
...
Phase E — Registry Infrastructure & Real Creds Management
...
E3. ✅ Per-user namespace isolation — image_ref format `{REGISTRY}/{owner_slug}/{name}:{tag}` via `_build_image_ref` in routes/instances.py:1760. `_owner_slug` is sha256(owner_id)[:16] (128-bit, B5 upgrade applied). Enforced at snapshot-create (owner can only push to their own slug) and at `GET /user-images/{id}` (owner check gates read). Registry-side ACL on ghcr.io is manual org-level config documented in docs/snapshot-registry.md.
E4. ✅ Platform-level docker login at host boot — `scripts/install.sh:219-300` `install_registry_login` writes `~/.xcelsior/registry.env` + `/etc/systemd/system/xcelsior-registry-login.service` (Type=oneshot, WantedBy=docker.service) so every GPU host auto-logs-into the registry on boot. Gracefully no-ops when `XCELSIOR_REGISTRY_URL` / username / password aren't set (dev workstations).
E5. ⏸ Per-user push tokens — requires a registry with programmatic scoped-credential issuance (Docker Registry v2 + auth-token server, or Harbor). ghcr.io only offers personal PATs with no scoping API; Harbor/registry-v2 deployment is a full infra epic (DB, LB, storage). Deferred until multi-tenant push is a concrete customer ask; current E3 slug namespacing + E4 single platform PAT is the industry-standard v1 posture (matches RunPod/Vast).
...
E7. ✅ Registry health metric — `registry_health.py` probes `{REGISTRY}/v2/` every 300s (bg_worker task `registry_health_probe`), exposes Prometheus gauges `xcelsior_registry_reachable{registry}`, `xcelsior_registry_last_probe_ts_seconds`, `xcelsior_registry_probe_latency_ms`. 401/403/404 count as reachable (HTTP stack up); only network errors count as down. `is_registry_healthy()` is O(1) module-scope cache used by E8.
E8. ✅ Graceful degradation (queue-on-registry-down) — snapshot endpoint in routes/instances.py:1897 checks `is_registry_healthy()` before enqueuing; if registry is down the row is created in `status='queued_registry_down'` instead of `pending`, and bg_worker task `snapshot_queue_retry` (60s) promotes queued rows to `pending` + enqueues the agent command as soon as health comes back. No lost snapshots during registry outages.
