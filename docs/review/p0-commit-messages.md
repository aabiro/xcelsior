...
<!-- Range origin/main..HEAD · 21 commits · head cae0460 -->
...
================================================================================
COMMIT b7b6c0b — docs(review): generate the commit transcript, and stop the guard flagging it
================================================================================

Two defects in one file, both from it being assembled by hand.

**It documented 16 commits while the branch carried 18** — omitting the commit
that added the file and the one after it. A transcript that silently drops
commits is worse than no transcript: `docs/review/p0-review.md` was written from
this file precisely so its citations could not drift from the branch, and a
reviewer reads an absence as evidence the work is not there.

`scripts/generate_review_commit_messages.py` makes it one command. The header it
emits records the range, the count and the head, so staleness is visible in the
file rather than inferred.

**Deliberately not added to `tests/test_generated_artifacts_are_current.py`.**
The endpoint inventory is gated there because it derives from the routes and
changes only when the surface does. This file derives from a *commit range*: it
changes on every commit and again whenever `main` moves, so a gate would fail
continuously until someone regenerated, which is how a gate becomes noise people
route around. The self-describing header is the weaker but honest alternative.

**The residency guard was failing on it**, and the reason is worth recording.
Line 810 quoted the gates commit explaining that *a text-scanning guard had
flagged the documentation of the defect it hunts* — so the residency guard
flagged the transcript for quoting that. Fifth instance of that recursion in this
suite; the previous count was four.

The generator now emits `residency-guard: documents-removal`, which is the
documented, greppable exemption for prose whose whole subject is the removal.
Applied to the marker rather than by widening the guard, because widening it
would exempt the file most likely to discuss the forbidden vocabulary — the
opposite of what the guard is for.

Branch suite goes from 5 failures to 4. The remaining four are the macOS
AppleDouble sidecars, fixed on `main` and arriving here with that merge.

Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>

================================================================================
COMMIT 929072a — Merge main into feat/mcp-p0-scopes — one guard, both writers, 403 for entitlement
================================================================================

Four decisions, not two conflicts. Recorded because each was a choice and none of
them was textual.

**`routes/auth.py` — the branch's mechanism, main's coverage.** `main` carried a
guard it labelled a stopgap: `_refuse_undelegatable_scopes` over a hand-written
`_OPERATOR_SCOPES_STOPGAP` frozenset. The branch carried `assert_scopes_delegable`,
which derives the operator set from the `(operator)` annotations in
`SCOPE_DESCRIPTIONS` so the set cannot drift from the consent text, and which also
refuses unknown scopes. The branch's mechanism is better and the stopgap is gone.

Safe only because the sets were compared first: **identical, seven scopes each,
no difference in either direction.** Had the derived set been missing one the
literal held, this would have reopened exactly that scope.

But the branch guarded **one** writer. `main` guarded two, and the second —
`PATCH /api/oauth/clients/{client_id}` — is the one closed and verified in
production. Registration alone leaves a two-request path to the same escalation:
register benignly, then amend. So `main`'s second call site is ported onto the
branch's mechanism, guarded for `scopes` absent (a body that does not mention
scopes must not be refused; `[]` is a real request and is checked).

**The status code, which a textual merge would have silently downgraded.** The
branch mapped every `OAuthGrantError` to 400. `main` answered 403, and
`tests/live/test_scope_refusals_live.py` asserts 403 *and* that the body names the
scope, against the deployed server. Flattening to 400 would have failed that gate
— which is what the gate is for.

Resolved as the distinction the code should have had: an unknown scope is a
malformed request (400); "you may not grant this" is an entitlement failure (403).
`test_unknown_scope_is_refused_at_registration` keeps 400 and still passes, so the
suite now encodes the difference rather than flattening it. RFC 7591's 400
convention governs the DCR endpoint in `oauth_registration.py`, not this
first-party management route.

**`routes/ssh.py` — the branch's version won on merit, then needed the same fix
twice.** It already enforced `ssh:write` on DELETE *and* restricted a machine
credential to deleting only keys it registered, which `main` lacked. But both of
its tests read `client_credentials` alone. Agent keys carry `auth_type:
"agent_api_key"` **and** a `client_id`, so the ownership restriction applies to
them uniformly — and without this, an agent key could have deleted a key its owner
pasted into the dashboard. Nobody had named that angle.

**Pair 3, in `_require_user_or_scoped_machine`:** `== "client_credentials"` →
`in _MACHINE_AUTH_TYPES`. A first attempt at this matched the pattern **twice**,
and the second occurrence was in `_require_user_grant`, where widening it would
have broken the fourteen callers that legitimately accept keys. Patched by line
instead; the assertion that caught it is the only reason.

**The two credential sets stay separate,** with the reasoning written between
them. `_MACHINE_AUTH_TYPES` asks whether a principal's scopes are an authorization
restriction (true for `client_credentials`, which is not a key at all);
`_API_KEY_AUTH_TYPES` asks whether this is a key rather than an interactive
session (true for legacy `api_key`, which has no scopes). They overlap on
`agent_api_key` because it is both. Collapsing them would either refuse
`client_credentials` from fourteen routes or stop enforcing scopes on the legacy
class.

**One gate of mine was wrong and is fixed here.**
`test_every_ssh_key_route_requires_its_scope` asserted the *spelling*
`_require_scope(user, X)`; the branch enforces the same rule through
`_require_user_or_scoped_machine(request, X)`. The rule held and the gate cried
wolf — a check pinned to wording rather than to the property, which is the defect
class this whole sequence has been about, in a gate written the day before. It now
accepts either enforcer.

Suite on the merged tree: 4836 passed, 18 skipped. The four macOS AppleDouble
failures the branch carried are gone, having been fixed on `main`.

Co-Authored-By: Claude Opus 5 <noreply@anthropic.com>

================================================================================
COMMIT cae0460 — Merge main (#30, the incident review) — docs only, no conflicts
================================================================================
