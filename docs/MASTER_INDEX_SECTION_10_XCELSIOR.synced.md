...
| 9 | - [x] Write the SCIP alignment one-pager + application scan (Canadian ownership ✓, 18-month ops plan = mesh + partner DC). | S6 F5.2
| 10 | - [x] Design `attestation` fields in host admission schema now (TEE evidence JWT, NRAS verify) so H100 hosts slot in without API breaks; recruit 1 H100 host partner; price the dedicated tier at 2–3× commodity. | S6 F5.2
