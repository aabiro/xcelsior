...
- **One policy registry. DONE** (2026-08-14). `contracts.ts`, `scopes.ts`, annotations, descriptions and `tool-surface.json` were five hand-maintained files that had to agree. `TOOL_SCOPE_REGISTRY` is now the source; its key set is exported as `ToolName` and every other per-tool table is `Record<ToolName, …>`, so a tool registered with no policy row, no description, or a row for a tool that does not exist, **fails to compile**. All three verified by injection. The 37-vs-39 drift is unrepresentable rather than detected.

  *Two corrections to how this was specified. "The rest are generated" is not what shipped, and generating them would have been wrong: `readOnly` and `destructive` are not derivable from a tool's scopes — an earlier attempt to derive `tenantClass` from scope prefixes misclassified `get_mcp_action_status` as operator because `hosts:read` appears in its alternatives, which would have pulled the customer's own "did my launch get approved?" tool from the public profile. They are hand-declared and **compiler-completeness-checked**, which is the guarantee the bullet actually wanted.*

  *And the inversion protects everything except the thing the neighbouring test file exists for. The compiler counts rows; it cannot read them. Nothing in the type system knows `instances:write` mutates, so `readOnly: true` beside a write scope type-checks. Worth stating because "the registry is inverted" reads like the whole class of drift is closed, and the coherence half is still a test's job.*

  *One measurement worth keeping: widening the registry from `satisfies Record<string, ScopeRequirement>` to an annotation of that type leaves `tsc` at **zero errors** while silently disabling every completeness check — the files all still look checked. Guarded, and verified red against that exact edit.*
...
- Eval baseline **captured** at **45 tools total / 36 published**. *The surface
  ...
  *Restated on 2026-08-14: **`TOOL_SCOPES` held 62 and
  ...
  matters — the feature was built and no agent could reach it. The create tool
  can launch sixty-four instances and is deliberately not a thinner gate than
  `create_instance`: it prepares a plan, the plan carries its member count, and
  the canonical hash binds it. Two eval cases were added with the tools, because
  `evals.test.ts` requires the case set to cover half the customer surface and
  adding tools without cases silently lowers that coverage.*

  *Restated from "39 tools", and the correction is two separate things. The
  count moved — `TOOL_SCOPES` held 43 and `tool-surface.json` published 34,
