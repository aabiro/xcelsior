...
def test_upgrade_agent_refuses_http_url(fake_agent_file):
    ...
    sha = "b" * 64
    assert worker_agent._handle_upgrade_agent({"url": "http://x/", "sha256": sha}) is False
...
def test_upgrade_agent_rejects_non_hex_sha(fake_agent_file):
    ...
    assert worker_agent._handle_upgrade_agent({"url": "https://x/", "sha256": bad}) is False
...
def test_upgrade_agent_rejects_oversized_body(fake_agent_file):
    ...
    with patch.object(worker_agent.requests, "get", return_value=_fake_response(200, huge)):
        ok = worker_agent._handle_upgrade_agent({"url": "https://x/", "sha256": sha})
    assert ok is False
    # File untouched
