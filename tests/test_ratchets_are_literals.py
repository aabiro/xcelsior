...
def test_the_known_ratchets_are_all_literals():
    ...
    # ── Three names removed from this set, and why it is not a weakening ──
    #
    # This file was recovered from `feat/mcp-p0-scopes`, whose pull request was
    # closed rather than merged. It named five ratchets; three live in guards
    ...
    #     test_tool_scope_registry_completeness  EXPECTED_TOOL_TOTAL
    #
    # Naming a ratchet whose file does not exist makes this fail permanently,
    # and permanently-failing is how a guard gets deleted rather than fixed. The
    # assertion's question — "was a guard deleted?" — has a real answer here:
    # no, it was never merged. That belongs in the record, not in a set that
    # cannot pass.
    #
    # When those guards land, their names come back into this set in the same
    # commit. The stranded files are enumerated in the commit that recovered
    # this one.
    seen = {
        (file, name)
