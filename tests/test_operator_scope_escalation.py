...
def test_non_admin_cannot_create_a_client_holding_an_operator_scope(scope):
    ...
    with pytest.raises(OAuthGrantError) as exc:
        assert_scopes_delegable([scope], creator=_non_admin())
...
def test_platform_admin_may_still_delegate_operator_scopes(scope):
    ...
    """
    from oauth_service import assert_scopes_delegable

    assert_scopes_delegable([scope], creator={"email": "a@x.ca", "is_admin": True})
...
def test_unknown_scopes_are_refused():
    ...
    with pytest.raises(OAuthGrantError) as exc:
        assert_scopes_delegable(["instance:read"], creator=_non_admin())
