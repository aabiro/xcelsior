...
const DESCRIPTIONS: Record<ToolName, string> = {
  ...
  list_providers:
    "List the provider accounts this credential can see — normally the caller's own. " +
    "Use when helping someone who supplies GPUs, before anything else: it is the only source of " +
    "the provider_id that get_provider_account and get_provider_earnings require, and ids are " +
    "not guessable. For a caller who is not a provider the list comes back empty, which is " +
